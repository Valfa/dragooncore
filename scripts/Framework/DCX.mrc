alias dcx {
  if (%dcx_conflict) { echo 4 -s [DCX ERROR] Dll Conflict, Trying to load multiple copies of DCX: $+(06,$qt($dll(dcx.dll)),) and $+(03,$qt(dll\dcx.dll),) | halt }
  if ($isid) returnex $dll(dll\dcx.dll,$1,$2-)
...
on *:start: {
  ...
  if ($dll(dcx.dll) != $null) {
    if ($v1 != dll\dcx.dll) {
      set %dcx_conflict 1
      echo 4 -s [DCX ERROR] Dll Conflict: Trying to load multiple copies of DCX: $+(04,$qt($v1),) and $+(03,$qt(dll\dcx.dll),)
    }
  }
}
